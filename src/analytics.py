...
# class Analytics:
#     """Analytics helper class for functionality related to viewing, analysing, and manipulating data from the SQLite database"""
...
#         """
#         Generates a statistics view per user for each library item.
...
#         db_user_stats: List[T_EmbyWatchHistStatsRow] = self.__cursor.fetchall()
#         self.__connector.commit()
        
#         # get item IDs and their string name
#         self.__cursor.execute("SELECT item_id, item_name FROM library_items")
#         db_lib_items: List[Tuple[str, str]] = self.__cursor.fetchall()
#         self.__connector.commit()

#         # grab users from Emby connector
#         users = EmbyConnector(debug=True).get_all_emby_users()


#         # load data into dataframe, rename column headers, calculate stats as above
#         df = pd.DataFrame(db_user_stats)
...
#         # build stats output into here - dict of dicts
#         # num_plays, total_minutes_watched, avg_fraction_completed, first_watch_at, last_watch_at
#         stats_df: Dict[str, Dict[int, float, float, str, str]] = pd.DataFrame
